...
# extract posteriors from Gompertz output
# This function is not working... fix this 2020-03-13!
extract.prediction.Gompertz <- function(zm, jags.data){
  ...
  pred.Y <- vector(mode = "list", length = length.b)
  k <- y <- c <- 1
  for (y in 1:length.year){    # years
    for (k in 1:jags.data$n.months){      # months
      pred.Y[[c]] <- b0[[c]] - b1[[c]] * exp(- b2 %*% t(Dvec))
      c <- c + 1
    }
  ...
  year.vec <- rep(seq(1, length.year), 
                      each = length(Dvec) * jags.data$n.months * length.year)
  
  b.vec <- rep(rep(seq(1, jags.data$n.months), 
                   each = length(Dvec)), 
               length.year)
...
```{r}
pred.list <- extract.prediction(zm = jm.1, jags.data = jags.data)
...
```{r}
MCMC.params$model.file = "models/Model_linear_Poly2_month_year.txt"
...
Of course... the 2nd order polynomial has to increase at the end, which is not good in this case. How about Gompertz? 

```{r}
MCMC.params$model.file = "models/Model_linear_Gompertz_month_year.txt"
...
# check some posteriors
mcmc_trace(jm.3$samples, pars = c("b0[1,1]", "b0[2,1]", "b0[3,1]", "b0[4,1]"))
mcmc_trace(jm.3$samples, pars = c("b1[1,2]", "b1[2,2]", "b1[3,2]", "b1[4,2]",
                                  "b1[5,2]", "b1[6,2]", "b1[7,2]"))
mcmc_trace(jm.3$samples, pars = c("b2"))
...
pred.list <- extract.prediction.Gompertz(zm = jm.3, jags.data = jags.data)
