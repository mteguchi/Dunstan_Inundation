...
nest.data %>% select(Month2, Year, Berm_Dist) %>%
  mutate(Year = ifelse(Year == 2018, 1, 2),
         Month_1 = Month2,
         Month_2 = Month2 + 1,
         Month_3 = Month2 + 2) -> new.data

new.data.1 <- new.data %>% transmute(Year = Year,
                                     Berm_Dist = Berm_Dist,
                                     Month = Month_1)
...
new.data.1 <- new.data %>% transmute(Year = Year,
                                     Berm_Dist = Berm_Dist,
                                     Month = Month_2)
pred.data.month2 <- predict.poly2.dataset.month(zm = jm.2, 
                                                new.data = new.data.1, 
                                                dataset_def = dataset_def)
...
pred.data.month3 <- predict.poly2.dataset.month(zm = jm.2, 
                                                new.data = new.data.1, 
                                                dataset_def = dataset_def)
...
n.inund.half <- n.inund.full  <- data.frame(Month1 = NA, Month2 = NA, Month3 = NA)
k <- 1
for(k in 1:length(pred.data.month1$pred.Y)){
  median.heights.1 <- as.numeric(list2DF(pred.data.month1$qtiles[[k]])[2,])
  upper.heights.1 <- median.heights.1 + (2 * median(sigma.y))  # sigma.y is from log scale
  nest.height <- as.numeric(log(nest.data[k, "Nest_Elevation"]))  # in log scale
  
  #n.inund.bottom[k, "Month1"] <- sum(upper.heights.1 < (nest.height+0.01) & upper.heights.1 > nest.height)
  n.inund.half[k, "Month1"] <- sum(upper.heights.1 > log(exp(nest.height) + nest.half))
  n.inund.full[k, "Month1"] <- sum(upper.heights.1 > log(exp(nest.height) + nest.full) )

  median.heights.2 <- as.numeric(list2DF(pred.data.month2$qtiles[[k]])[2,])
  upper.heights.2 <- median.heights.2 + (2 * median(sigma.y))
  #n.inund.bottom[k, "Month2"] <- sum(upper.heights.2 < (nest.height+0.01) & upper.heights.2 > nest.height)
  n.inund.half[k, "Month2"] <- sum(upper.heights.2 > log(exp(nest.height) + nest.half) )
  n.inund.full[k, "Month2"] <- sum(upper.heights.2 > log(exp(nest.height) + nest.full) )
  
  if (k <= length(pred.data.month3$pred.Y)){
    median.heights.3 <- as.numeric(list2DF(pred.data.month3$qtiles[[k]])[2,])
    upper.heights.3 <- median.heights.3 + (2 * median(sigma.y))
    #n.inund.bottom[k, "Month3"] <- sum(upper.heights.3 < (nest.height+0.01) & upper.heights.3 > nest.height)
    n.inund.half[k, "Month3"] <- sum(upper.heights.3 > log(exp(nest.height) + nest.half))
    n.inund.full[k, "Month3"] <- sum(upper.heights.3 > log(exp(nest.height) + nest.full) )
    
  }
  
}

nest.data %>% select(fid, Nest_UID, Northings, Eastings, Nest_Elevation, Berm_Dist, Total_clutch, 
                     Habitat, rate_success, Year, Month2, Habitat, Phase_1_death, Month_f)  -> nest.data.2

# nest.data.2$n.inund.bottom.month1 <- n.inund.bottom$Month1
# nest.data.2$n.inund.bottom.month2 <- n.inund.bottom$Month2
# nest.data.2$n.inund.bottom.month3 <- n.inund.bottom$Month3
nest.data.2$n.inund.half.month1 <- n.inund.half$Month1
nest.data.2$n.inund.half.month2 <- n.inund.half$Month2
nest.data.2$n.inund.half.month3 <- n.inund.half$Month2
nest.data.2$n.inund.full.month1 <- n.inund.full$Month1
nest.data.2$n.inund.full.month2 <- n.inund.full$Month2
nest.data.2$n.inund.full.month3 <- n.inund.full$Month3
...
#  facet_grid(Year ~ Month_f)  +
     geom_point(data = nest.data.2,
              aes(x = Berm_Dist,
                  ...
                  color = as.factor(n.inund.full.month1)),
              alpha = 0.5) +
  facet_grid(Year ~ Month_f) + 
...
#  facet_grid(Year ~ Month_f)  +
     geom_point(data = nest.data.2,
              aes(x = Berm_Dist,
                  y = log(Nest_Elevation + nest.full),
                  color = as.factor(n.inund.full.month2)),
              alpha = 0.5) +
  facet_grid(Year ~ Month_f) + 
...
#  facet_grid(Year ~ Month_f)  +
     geom_point(data = nest.data.2,
              aes(x = Berm_Dist,
                  y = log(Nest_Elevation + nest.full),
                  color = as.factor(n.inund.full.month3)),
              alpha = 0.5) +
  facet_grid(Year ~ Month_f) + 
  labs(title = "Predicted tide height and # inundation during third month") + 
  guides(fill = FALSE) + 
  scale_color_discrete(name = "# inund")
...
Nest elevation, # inundation half way of the nest during the second and third months, and distance from the berm are important.
