...
n.inund.bottom <- n.inund.half <- n.inund.full  <- data.frame(Month1 = NA, Month2 = NA, Month3 = NA)
k <- 1
for(k in 1:length(pred.data.month1$pred.Y)){
  median.heights.1 <- as.numeric(list2DF(pred.data.month1$qtiles[[k]])[2,])
  upper.heights.1 <- median.heights.1 + (2 * median(sigma.y))
  nest.height <- as.numeric(log(nest.data[k, "Nest_Elevation"]))
  
  n.inund.bottom[k, "Month1"] <- sum(upper.heights.1 < (nest.height+0.01) & upper.heights.1 > nest.height)
  n.inund.half[k, "Month1"] <- sum(upper.heights.1 < (nest.height + 0.3) & upper.heights.1 > nest.height)
  n.inund.full[k, "Month1"] <- sum(upper.heights.1 < (nest.height + 0.6) & upper.heights.1 > (nest.height+0.3))
  ...
  n.inund.half[k, "Month2"] <- sum(upper.heights.2 < (nest.height + 0.3) & upper.heights.2 > nest.height)
  n.inund.full[k, "Month2"] <- sum(upper.heights.2 < (nest.height + 0.6) & upper.heights.2 > (nest.height+0.3))
  ...
  if (k <= length(pred.data.month3$pred.Y)){
    ...
    n.inund.half[k, "Month3"] <- sum(upper.heights.3 < (nest.height + 0.3) & upper.heights.3 > nest.height)
    n.inund.full[k, "Month3"] <- sum(upper.heights.3 < (nest.height + 0.6) & upper.heights.3 > (nest.height+0.3))
    
  }
  
}

nest.data %>% select(Northings, Eastings, Nest_Elevation, Berm_Dist, Total_clutch, 
                     Habitat, rate_success, Year, Month2, Habitat) -> nest.data.2
...
M.1 <- as.formula("rate_success ~ Berm_Dist + 
                  ...
                  Northings +
                  Eastings +
                  n.inund.bottom.month1 + 
                  n.inund.bottom.month2 + 
                  n.inund.bottom.month3 + 
                  n.inund.half.month1 + 
                  n.inund.half.month2 + 
                  n.inund.half.month3 + 
                  n.inund.full.month1 + 
...
Nest elevation, # inundation half way of the nest during the first months, and distance from the berm are important.
