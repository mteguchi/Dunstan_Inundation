...
It looks okay but ...Of course... the 2nd order polynomial has to increase at the end, which may not be a good feather in this case. 
...
pred.list.jm.3$pred.df %>% 
  mutate(Month = ifelse(Month2 < 4, Month2 + 9, Month2 - 3)) -> pred.df.jm.3
...
ggplot() + 
  geom_point(data = inund.2018,
             aes(x = scaled_Dist_Berm,
                 y = log(InundHeight_AbvLAT))) +
  geom_path(data = pred.df.jm.3.2018,
            aes(x = Dist_Berm,
                 y = (pred_med))) +
  geom_ribbon(data = pred.df.jm.3.2018,
              aes(x = Dist_Berm,
                  ymin = (pred_low),
                  ymax = (pred_high)),
              alpha = 0.5) +
...
pred.df.jm.3 %>% filter(Year == 2019) -> pred.df.jm.3.2019
ggplot() + 
  geom_point(data = inund.2019,
             aes(x = scaled_Dist_Berm,
                 y = log(InundHeight_AbvLAT))) +
  geom_path(data = pred.df.jm.3.2019,
            aes(x = Dist_Berm,
                 y = (pred_med))) +
  geom_ribbon(data = pred.df.jm.3.2019,
              aes(x = Dist_Berm,
                  ymin = (pred_low),
                  ymax = (pred_high)),
              alpha = 0.5) +
...
pred.list.jm.4$pred.df %>% mutate(Month = ifelse(Month2 < 4, Month2 + 9, Month2 - 3)) -> pred.df.jm.4
...
ggplot() + 
  geom_point(data = inund.2018,
             aes(x = scaled_Dist_Berm,
                 y = log(InundHeight_AbvLAT))) +
  geom_path(data = pred.df.jm.4.2018,
            aes(x = Dist_Berm,
                 y = (pred_med))) +
  geom_ribbon(data = pred.df.jm.4.2018,
              aes(x = Dist_Berm,
                  ymin = (pred_low),
                  ymax = (pred_high)),
              alpha = 0.5) +
...
```{r}
pred.df.jm.4 %>% filter(Year == 2019) -> pred.df.jm.4.2019

ggplot() + 
  geom_point(data = inund.2019,
             aes(x = scaled_Dist_Berm,
                 y = log(InundHeight_AbvLAT))) +
  geom_path(data = pred.df.jm.4.2019,
            aes(x = Dist_Berm,
                 y = (pred_med))) +
  geom_ribbon(data = pred.df.jm.4.2019,
              aes(x = Dist_Berm,
                  ymin = (pred_low),
                  ymax = (pred_high)),
              alpha = 0.5) +
...
pred.list.jm.5$pred.df %>% mutate(Month = ifelse(Month2 < 4, Month2 + 9, Month2 - 3)) -> pred.df.jm.5
...
ggplot() + 
  geom_point(data = inund.2018,
             aes(x = scaled_Dist_Berm,
                 y = log(InundHeight_AbvLAT))) +
  geom_path(data = pred.df.jm.5.2018,
            aes(x = Dist_Berm,
                 y = (pred_med))) +
  geom_ribbon(data = pred.df.jm.5.2018,
              aes(x = Dist_Berm,
                  ymin = (pred_low),
                  ymax = (pred_high)),
              alpha = 0.5) +
...
```{r}
pred.df.jm.5 %>% filter(Year == 2019) -> pred.df.jm.5.2019

ggplot() + 
  geom_point(data = inund.2019,
             aes(x = scaled_Dist_Berm,
                 y = log(InundHeight_AbvLAT))) +
  geom_path(data = pred.df.jm.5.2019,
            aes(x = Dist_Berm,
                 y = (pred_med))) +
  geom_ribbon(data = pred.df.jm.5.2019,
              aes(x = Dist_Berm,
                  ymin = (pred_low),
                  ymax = (pred_high)),
              alpha = 0.5) +
...
DIC is best for jm.2 (2nd order polynomial).   
```{r}
c(jm.1$DIC, jm.2$DIC, jm.3$DIC, jm.4$DIC, jm.5$DIC)
```
