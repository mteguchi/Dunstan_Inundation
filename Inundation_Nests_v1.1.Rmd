...
if (!file.exists("RData/RF_15d_M_1_out.rds")){
...
Rate success is most important. 


Compare the data and predictions:

```{r}
nest.data.30d$PREDICTED <- pred.RF.M.1[,1]
ggplot(data = nest.data.30d) +
  geom_point(aes(x = Phase_1_death, y = PREDICTED)) + 
...
Maybe there is a little bit of declining as dif.nest.pred increases? 

```{r}

nest.data.45d %>%
  mutate(logit.phase.1 = car::logit(Phase_1_death, adjust = 0.025)) -> nest.data.45d

fit.1 <- lm(logit.phase.1 ~ dif.nest.pred, data = nest.data.45d)

summary(fit.1)
```

A neagative relationship but not very convincing... 
