...
Found that this is not very useful... some errors happen due to the changed variable names in the original data file. 
...
inund.cols <- cols(fid_ = col_integer(),
                   ...
                   Incubation = col_date(format = "%Y-%m-%d"),
...
inund.1 <- read_csv(file = "data/inund_results_v2.csv",
...
ggplot(data = zero.inund) + 
  geom_histogram(aes(x = prop_succe),
                 bins = 10) +
...
only.inund %>% select(Nest_Eleva, Berm_Dist, Season, Sector, Northings, Eastings,
                      Hatched, Live_hatch, Total_clut, prop_succe,
                      Max_Height.x, n_Inundati, Max_Inund_Duration_Hr,
                      ...
                      inund_hrs_gt_75) %>%
  mutate(p_success = (Hatched + Live_hatch)/Total_clut,
...
only.inund.2 %>% select(p_success, Max_Height.x, 
                        n_Inundati, Max_Inund_Duration_Hr,
                        Total_Inund_Duration_Hr, Mean_Inund_Duration_Hr,
                        Prop_Inund_Duration, SD_Inund_Duration_Hr, Mean_Max_Height_Per_Event,
                        SD_Max_Height_Per_Event, n_Inund_Phase_A, n_Inund_Phase_B,
                        n_Inund_Phase_C, Hr_Inund_Phase_A, Hr_Inund_Phase_B, Hr_Inund_Phase_C,
...
<!-- Throw in all variables - not enough data points to use RF here...  -->

<!-- ```{r, echo=FALSE, results='hide'} -->
<!-- response <- only.inund.2$prop_succe/100 -->

<!-- predictors <- only.inund.2 %>% select(-c(p_success, prop_succe, f_Season, Season, f_Sector, Sector)) -->
...
<!-- M.3 <- as.formula("p_success ~ Nest_Eleva +  -->
<!--                   Berm_Dist +  -->
<!--                   n_Inundati +  -->
...
<!-- fit.1 <- lm(M.3, data = RF.data) -->
<!-- summary(fit.1) -->

<!-- # RF is not working... 2021-05-11 -->
...
<!--          Max_Height_Berm_Dist = Berm_Dist * Max_Height.x, -->
<!--          Max_Height = Max_Height.x, -->
...
fit.betareg.1 <- betareg(p.success.2 ~ Berm_Dist + Max_Height.x + Mean_Max_Height_Per_Event + Hr_Inund_Phase_B +  Nest_Eleva + f_Season,
                         data = betareg.data)
...
```{r}
predictors <- only.inund.2 %>% select(-c(p_success, f_Season, Season, f_Sector, Sector))
predictors %>% mutate(Max_Height_Berm_Dist = Max_Height.x * Berm_Dist,
...
jags.data <- list(y = response,
                  X = cbind(1, Bayes.data$Max_Height.x,
                            Bayes.data$Mean_Max_Height_Per_Event,
                            Bayes.data$Berm_Dist,
...
R2.V4.interactions <- bayes.logit.R2(jags.data$X,
                                     betas, 
                                     jm.V4.interactions$samples, "sigma.y")
...
```{r}
bayesplot::mcmc_dens(jm.V4.interactions$samples, 
