...
fit.betareg.1 <- betareg(p.success.2 ~ Berm_Dist + Max_Height + Mean_Max_Height_Per_Event + Hr_Inund_Phase_B +  f_Season,
                         data = betareg.data)

summary(fit.betareg.1)
```

Distance from the berm and hours of inundation during phase B (18-36 days of incubation; middle third) seem to affect the proportion of hatching success. 

Including more predictors doesn't help tune the model.

```{r}
fit.betareg.2 <- betareg(p.success.2 ~ Berm_Dist + Max_Height + Mean_Max_Height_Per_Event + Hr_Inund_Phase_B ,
                         data = betareg.data)
...
Season has the smallest f value. so let's remove it...
```{r}
jags.data <- list(y = response,
                  X1 = Bayes.data$Max_Height,
                  X2 = Bayes.data$Mean_Max_Height_Per_Event,
...
Berm_Dist is the lowest

```{r}
jags.data <- list(y = response,
                  X1 = Bayes.data$Max_Height,
                  X2 = Bayes.data$Mean_Max_Height_Per_Event,
                  X3 = Bayes.data$Berm_Dist,
                  N = nrow(Bayes.data),
                  n.pred = 3)
...
The simplest one is the best according to the DIC.
...
So the best model included 3 predictors: Max_Height, Mean_Max_Height_Per_Event, and Berm_Dist.

```{r}
jm.V3$summary %>% as.data.frame %>% rownames_to_column("parameter") -> tmp

tmp[grep("b", tmp$parameter),] %>% 
  select(-c(overlap0, f)) -> tmp2
...
```{r}
bayesplot::mcmc_trace(jm.V3$samples, c("b0", "b[1]", "b[2]", "b[3]"))
```


```{r}
bayesplot::mcmc_dens(jm.V3$samples, c("b0", "b[1]", "b[2]", "b[3]"))
```
...
X <- Bayes.data %>% 
  select(Max_Height, Mean_Max_Height_Per_Event, Berm_Dist) %>% 
  as.matrix() 
