...
for (k1 in 1:length(inundated.nests)){
  ...
  inundated.nests[[k1]] <- list(nest.ID = nest.ID,
                                ...
                                hr.inund.phase2 = sum(inund.duration.hr[inund.time.phase > 18/51 & inund.time.phase <= 37/51]),
...
inund %>% left_join(out.df, by = "Nest_UID") %>%
  select(-c("Number_inundation_events", 
            "Inundation_period_per_event",
            "Phase_incubation_per_inundation", 
            "Max_inundation_height_event", 
            "Duration_max_inundation")) -> inund.2
