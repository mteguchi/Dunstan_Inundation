...
logger.3.1 <- read_csv("data/Logger3_1.csv",
                       col_types = logger.cols) %>%
  mutate(Logger_ID = "3.1",
...
logger.5.4 <- read_csv("data/Logger5_4.csv",
  ...
  mutate(Logger_ID = "5.4",
         Logger_Depth = min(Logger_Depth, na.rm = TRUE),
         Date = as.Date(Date_Time, format = "%Y-%m-%d"))
loggers <- rbind(logger.3.1, logger.3.2, logger.4.1, logger.4.2, logger.4.3,
...
for (k1 in 1:length(inundated.nests)){
  ...
  if (sum(logger.k$inund) == nrow(logger.k)){
    ...
    inund.duration.hr <- difftime(logger.k$Date_Time[nrow(logger.k)], logger.k$Date_Time[1], units = "hours")
    #inund.duration.unit <- units(logger.k$Date_Time[nrow(logger.k)] - logger.k$Date_Time[1])
    max.height.per.event <- max(logger.k$Height_LAT)
    max.height <- max.height.per.event
    inund.time.day <- NA
    inund.time.phase <- NA
    max.inund.duration.hr <- NA
    ...
    if (nrow(logger.k.inund) > 1){
      ...
      #inund.duration.unit <- vector(mode = "character", length = n.inund)
      ...
      for (k3 in 1:n.inund){
        ...
        inund.duration.hr[k3] <- difftime(inund.tmp$Date_Time[nrow(inund.tmp)], inund.tmp$Date_Time[1],
                                          units = "hour")
        #inund.duration.unit[k3] <- units(inund.tmp$Date_Time[nrow(inund.tmp)] - inund.tmp$Date_Time[1])
        inund.time.day[k3] <- difftime(inund.tmp$Date_Time[nrow(inund.tmp)], logger.k$Date_Time[1],
                                       units = "days")
      ...
      # duration of the maximum inundation 
      max.inund.duration.hr <- inund.duration.hr[max.height.per.event == max(max.height.per.event)]
      #max.inund.duration.unit <- inund.duration.unit[max.height == max(max.height)]
      ...
      #inund.duration.unit <- "minute"
      inund.time.day <- difftime(logger.k.inund$Date_Time, logger.k$Date_Time[1], units = "days")
  ...
  inundated.nests[[k1]] <- list(nest.ID = nest.ID,
                                ...
                                max.inund.duration.hr = max.inund.duration.hr)
}
```
...
out.df <- data.frame(Nest_UID = unlist(lapply(inundated.nests, 
                    ...
                    #                                   function(x) x$date.end)),
                     ...
                     Max_Inund_Duration_Hr = unlist(lapply(inundated.nests, 
                                                           function(x) x$max.inund.duration.hr)))
