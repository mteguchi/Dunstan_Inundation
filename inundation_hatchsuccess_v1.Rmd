...
ADD NEST ID, INUNDATION EVENT ID IN OUTPUT. RECONSTRUCT THE LIST WITH NEW LOGGER.K AND LOGGER.K.INUND.
2020-12-08
```{r}
...
for (k1 in 1:length(inundated.nests)){
  ...
  if (sum(logger.k$inund) == nrow(logger.k)){
    inundated.nests[[k1]]$n.inund <- 1
    inundated.nests[[k1]]$time.inund <- logger.k$Date_Time[nrow(logger.k)] -
                                                        logger.k$Date_Time[1]
    inundated.nests[[k1]]$max.height <- max(logger.k$Height_LAT)
    inundated.nests[[k1]]$phase <- NA
    inundated.nests[[k1]]$days.max.inund <- NA
    
  } else {  # if there were at least one un-inundated reading, we need to figure out inundation events
    #duration.inund <- vector(mode = "numeric")
    ...
    if (nrow(logger.k.inund) > 1){
      ...
      n.inund <- max(tmp)
      inundated.nests[[k1]]$n.inund <- n.inund
      ...
      inundated.nests[[k1]]$max.height <- max(logger.k.inund$Height_LAT)
      # Rather than "phase" I use # days from start date
      inund.phase <- vector(mode = "numeric", length = n.inund)
      inund.phase.unit <- vector(mode = "character", length = n.inund)
      k3 <- 1
      ...
      for (k3 in 1:n.inund){
        ...
        inund.phase[k3] <- inund.tmp$Date_Time[nrow(inund.tmp)] - logger.k$Date_Time[1]
        inund.phase.unit[k3] <- units(inund.tmp$Date_Time[nrow(inund.tmp)] - logger.k$Date_Time[1])
      }
      
      inundated.nests[[k1]]$time.inund <- time.inund
      inundated.nests[[k1]]$time.inund.unit <- time.inund.unit
      inundated.nests[[k1]]$inund.phase <- inund.phase
      inundated.nests[[k1]]$inund.phase.unit <- inund.phase.unit
      inundated.nests[[k1]]$days.max.inund <- NA
      
    } else {
      inundated.nests[[k1]]$max.height <- logger.k.inund$Height_LAT
      inundated.nests[[k1]]$n.inund <- 1
      inundated.nests[[k1]]$time.inund <- 29
      inundated.nests[[k1]]$time.inund.unit <- "minute"
      inundated.nests[[k1]]$inund.phase <- logger.k.inund$Date_Time - logger.k$Date_Time[1]
      inundated.nests[[k1]]$inund.phase.unit <- units(logger.k.inund$Date_Time - logger.k$Date_Time[1])
      inundated.nests[[k1]]$days.max.inund <- NA
